...
test('真实图片上传、移动端输入聚焦与 OCR 失败恢复', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '拍题' }).click()
...
test('异常网络不白屏，真实部署域名同源访问正常', async ({ browser }) => {
  ...
  await networkPage.getByRole('button', { name: '练习' }).click()
  ...
  await domainPage.getByRole('button', { name: '拍题' }).click()
