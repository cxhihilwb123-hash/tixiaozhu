...
const assertProductionLaunchAllowed = () => {
  if (!IS_PRODUCTION_RUNTIME || process.env.ALLOW_BLOCKED_PRODUCTION_START === 'true') return
...
const adminCredentialConfig = () => ({
  ...
  password: String(process.env.ADMIN_PASSWORD || store.settings.adminPassword || 'admin123'),
})
...
const adminLogin = (body) => {
  ...
  const { username: expectedUsername, password: expectedPassword } = adminCredentialConfig()
...
const createStudentPasswordHash = (password, salt = randomBytes(16).toString('hex')) => {
...
const verifyStudentPassword = (password, storedHash) => {
  ...
  const expected = createStudentPasswordHash(password, salt).split('.')[1]
...
const bindStudentBody = (req, body = {}) => {
  ...
  if (!token && !REQUIRE_STUDENT_AUTH) return body
...
const registerStudent = (body) => {
  ...
  const authUser = {
    ...
    phone,
    nickname: profile.nickname,
    passwordHash: createStudentPasswordHash(password),
    provider: 'password',
    status: 'active',
...
const loginStudent = (body) => {
  ...
  if (!authUser || authUser.status === 'disabled' || !verifyStudentPassword(password, authUser.passwordHash)) {
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (path === '/api/auth/register' && req.method === 'POST') {
      return json(res, 201, ok(registerStudent(await readBody(req))))
    }

    if (path === '/api/auth/login' && req.method === 'POST') {
      return json(res, 200, ok(loginStudent(await readBody(req))))
    }
    ...
    if (path === '/api/admin/auth/login' && req.method === 'POST') {
      return json(res, 200, ok(adminLogin(await readBody(req))))
    }
    ...
    if (path === '/api/payments/mock-confirm' && req.method === 'POST') {
      ...
      return json(res, 200, ok(confirmPayment(await readBody(req))))
    }
